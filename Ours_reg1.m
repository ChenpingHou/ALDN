function [W_best,alpha_best] = Ours_reg1(vanish_rate,noise_M,W1,T,data_s2, label_s2,noise_label_s2,alpha_set)
%% Input:
...
while (abs(cost_old - cost) > 10^-6 && count < loop_max )
    ...
    cost = -1/nSam2 * cost + alpha*sum((W2(:)-W0(:)).^2);
    W2_grad = -1/nSam2 * W2_grad + 2*alpha*(W2-W0);
...
cost_after = -1/nSam2 * cost + alpha*sum((W2_temp(:)-W0(:)).^2);
if isnan(cost_after)
    cost_after = 10^8;
end

function [W_best,alpha_best,object_value] = IA(vanish_rate,W1,T,data_s2, label_s2,noise_label_s2,alpha_set)
% label:c*n
% feature:d*n
% w:c*d
% alpha_best = 10^[-3:1:3];
...
while (abs(cost_old - cost) > 10^-6 && count < loop_max )
    ...
    D = diag(MM);
    cost = -1/nSam2 * noise_label(:)' * log(p(:)) + alpha*sum(norm_column(1:end));
    W2_grad = -1/nSam2 * (noise_label - p) * data' + alpha* (data*D*(W2*data-W0*data)')';  
...
cost_after = -1/nSam2 * noise_label(:)' * log(p(:)) + alpha*sum(norm_column(1:end));
if isnan(cost_after)
    cost_after = 10^8;
end
